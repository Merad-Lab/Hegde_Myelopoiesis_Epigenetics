...
enr_ChIPX_up_results <- enr_ChIPX_up_results %>%
  subset(Term != "TCF3 CHEA") %>%
  subset(Term != "STAT3 CHEA") %>%
  subset(Term != "SPI1 ENCODE")
...
ggplot(enr_ChIPX_up_results, aes(y = Odds.Ratio, x = -log10(Adjusted.P.value))) +
  ...
  geom_text_repel(
    data = subset(enr_ChIPX_up_results, label %in% c("NFE2L2",'RELA','TCF3', 'CHD1', 'GATA1', 'SOX2', 'GATA2', 'STAT3', 'ZNF384', 'RUNX1', 'SPI1', 'CEBPD',
                                                     'SMAD4', 'TAF7', 'BHLHE40','VDR', 'NELFE','PPARG','TAF1')), 
    aes(label = label),
    nudge_y = -0.25,
    nudge_x = 0.35,
    color = "black",
